...
export async function GET(request: NextRequest) {
  try {
      ...
      if (ids.length > 1) {
        ...
        for (const content of contents) {
          ...
          const sanitizedTitle = (seoMeta.title || content.topic_id || content.content_id || 'article')
            ...
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/^-+|-+$/g, '')
          const filename = `${sanitizedTitle}-${content.content_id}.html`
        ...
        return new NextResponse(zipBuffer as unknown as BodyInit, {
          ...
          headers: {
            ...
            'Content-Disposition': 'attachment; filename="articles-html.zip"',
    ...
    // Single content ID (from contentId param or from contentIds with one id)
    ...
    console.log(`✅ Downloading HTML for content_id: ${effectiveContentId}, title: "${seoMetaForLog.title || 'N/A'}"`)
    ...
    const filename = `${sanitizedTitle}-${effectiveContentId}.html`
